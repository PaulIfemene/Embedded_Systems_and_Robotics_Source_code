...
// ------------------- Google Script URL -------------------
// (your deployed Apps Script web app that updates the sheet)
const char* googleScriptURL = "https://script.google.com/macros/s/AKfycbxiM0Wx_qi4KfQzp-Rt0mdxKFRetvzA0awfqyxAb6yMvC69i7gV3lzHxIPcrXyXX3DP/exec";
...
void setup() {
  ...
  // Initialize NTP after Wi-Fi (if connected)
...
void registerUser() {
  ...
  if (enrollFingerprint(id)) {
    ...
    // Send registration event to Google Sheets
    ...
    // Optional: send failure event or log locally
  }
}
    ...
    if (result == FINGERPRINT_OK) {
      ...
      for (uint8_t i = 0; i < userCount; i++) {
        if (users[i].id == fid) {
          ...
          // Send verification to Google Sheets
      ...
      if (!found) {
        ...
        // Optionally send unknown fingerprint event with ID
        String timestamp = getTimeStamp();
...
void sendToGoogleSheet(String name, String regNo, uint8_t fid, String timestamp, String status) {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("Wi-Fi not connected — cannot send to Google Sheets now.");
    return;
  }
  ...
  if (!success) {
    Serial.println("Failed to send data to Google Sheets after retries.");
    // Optional: Save to SD / SPIFFS for later upload
  }
}
