...
// ======= GOOGLE SCRIPT URL =======
const char* googleScriptURL = "https://script.google.com/macros/s/AKfycbxiM0Wx_qi4KfQzp-Rt0mdxKFRetvzA0awfqyxAb6yMvC69i7gV3lzHxIPcrXyXX3DP/exec"; // replace with your /exec link
...
void sendToGoogleSheet(String name, String regNo, String status) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("⚠️ Wi-Fi disconnected, cannot send data now.");
  }
}
...
void setup() {
  ...
  // ======= STEP 1: Initialize Fingerprint FIRST =======
  Serial.println("Initializing Fingerprint sensor...");
  int retries = 0;
  while (!finger.verifyPassword()) {
    Serial.println("❌ Fingerprint sensor not detected! Retrying...");
    retries++;
    delay(1000);
    if (retries > 5) {
      Serial.println("❌ Could not detect fingerprint sensor. Restart device.");
      while(1) delay(1);
    }
  }
  Serial.println("✅ Fingerprint sensor detected!");

  // ======= STEP 2: Connect to Wi-Fi AFTER fingerprint ready =======
  ...
  while (WiFi.status() != WL_CONNECTED) {
    ...
    Serial.print(".");
    wifiRetries++;
    if (wifiRetries > 60) { // ~30 seconds timeout
      Serial.println("\n⚠️ Failed to connect Wi-Fi, retry device restart.");
      while(1) delay(1);
    }
  }
  Serial.println("\n✅ Wi-Fi Connected!");
  ...
  // ======= STEP 3: Initialize NTP for timestamps =======
